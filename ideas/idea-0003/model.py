...
class ModelConfig:
    idea_id: str = "idea-0003"
    architecture_name: str = "idea_0003_creative_lm"
...
class CreativeIdeaModel(nn.Module if nn is not None else object):
    def __init__(self, config: ModelConfig) -> None:
        if nn is not None:
            super().__init__()
        self.config = config
        self.architecture_rationale = ['Combine non-default mechanisms around: invent, topology-mixed, language, recurrent', 'Reject template decoder-only stacks with only cosmetic changes.', 'Use explicit originality rationale before implementation starts.']
        if nn is not None:
